...
export async function loadGuestbook() {
  ...
  try {
    var res = await state.dashboardSb.from("guestbook").select("id, nama, pesan, is_approved, created_at").order("created_at", { ascending: false }).limit(500);
...
function renderGbList() {
  ...
  container.innerHTML = "";
  var filtered = state.allGb;
  if (state.gbFilter === "pending") filtered = state.allGb.filter(function(e) { return !e.is_approved; });
  else if (state.gbFilter === "approved") filtered = state.allGb.filter(function(e) { return e.is_approved; });
  document.getElementById("gb-empty").classList.toggle("d-none", filtered.length !== 0);
...
export async function toggleGbApproval(id, newVal, btn) {
  btn.disabled = true; var origText = btn.textContent; btn.textContent = "Memproses...";
  try {
    var res = await state.dashboardSb.from("guestbook").update({ is_approved: newVal }).eq("id", id);
    if (res.error) throw res.error;
    var entry = state.allGb.find(function(e) { return e.id === id; });
    if (entry) entry.is_approved = newVal;
    renderGbList();
    showToast(newVal ? "Pesan ditampilkan." : "Pesan disembunyikan.");
  } catch(err) { showToast("Gagal update.", true); btn.disabled = false; btn.textContent = origText; }
}

export function setGbFilter(filter, btn) {
  state.gbFilter = filter;
  document.querySelectorAll("#tab-guestbook .btn-group button").forEach(function(b) { b.classList.remove("active"); });
