// src/main/utils.js — ES module version
...
export function showRsvpModal(msg, isError) {
...
function getRsvpModalOverlay() {
  ...
  overlay.addEventListener("click", function (e) { if (e.target === overlay) hideRsvpModal(); });
  overlay.querySelector(".rsvp-modal-close").addEventListener("click", hideRsvpModal);
  document.addEventListener("keydown", function (e) { if (e.key === "Escape") hideRsvpModal(); });
  return overlay;
}
...
export function copyToClipboard(text) {
  navigator.clipboard.writeText(text)
    ...
    .catch(function () { prompt("Salin nomor rekening:", text); });
}
...
export function createPageItem(html, enabled, onClick, isActive) {
  ...
  a.href = "#";
  a.innerHTML = html;
  a.addEventListener("click", function (e) { e.preventDefault(); if (enabled) onClick(); });
  li.appendChild(a);
  return li;
}
...
export function renderPagination(config) {
  ...
  container.innerHTML = "";
  if (totalPages <= 1) { container.classList.add("d-none"); return; }
  container.classList.remove("d-none");
  var ul = document.createElement("ul");
  ul.className = "pagination justify-content-center";
  var prevLi = createPageItem('<span aria-hidden="true">&laquo;</span>', currentPage > 0, function () { if (currentPage > 0) onPageChange(currentPage - 1); });
  ul.appendChild(prevLi);
  for (var i = 0; i < totalPages; i++) {
    ul.appendChild(createPageItem(String(i + 1), true, (function (p) { return function () { onPageChange(p); }; })(i), i === currentPage));
  }
  var nextLi = createPageItem('<span aria-hidden="true">&raquo;</span>', currentPage < totalPages - 1, function () { if (currentPage < totalPages - 1) onPageChange(currentPage + 1); });
