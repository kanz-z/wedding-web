...
serve(async (req) => {
  ...
  try {
    ...
    const { guest_id, nama, nomor_wa, jumlah_hadir, status, pesan, qr_token } = body;
    ...
    if (pesan && pesan.length > 500) {
      return new Response(JSON.stringify({ error: "Pesan terlalu panjang, maksimal 500 karakter" }), {
        status: 400,
        headers,
      });
    }
    ...
    if (isNaN(hadir) || hadir < 1) {
      return new Response(JSON.stringify({ error: "Jumlah hadir tidak valid" }), {
        status: 400,
        headers,
      });
    }
