// dashboard batch operations & import
...
async function executeImport() {
  ...
  for (var i = 0; i < lines.length; i++) {
    ...
    var guestRes = await dashboardSb
      .from("guests")
      .insert([
        {
          slug: slug,
    ...
    var rsvpRes = await dashboardSb.from("rsvps").insert([
      {
    ...
    if (rsvpRes.error) {
      await dashboardSb.from("guests").delete().eq("id", guestRes.data.id);
...
async function executeBatchDelete(selected) {
  ...
  try {
    ...
    var rsvpRes = await dashboardSb.from("rsvps").select("id").in("guest_id", guestIds);
    ...
    if (rsvpRes.data && rsvpRes.data.length > 0) {
      ...
      await dashboardSb.from("guest_checkins").delete().in("rsvp_id", rsvpIds);
      await dashboardSb.from("rsvps").delete().in("guest_id", guestIds);
    }

    await dashboardSb.from("guests").delete().in("id", guestIds);
...
async function generateSingleCard(guest) {
  ...
  if (!token) {
    ...
    var updateRes = await dashboardSb
      .from("rsvps")
