...
document
  ...
  .addEventListener("submit", async function (e) {
    ...
    try {
      var guestId = id;
      if (id) {
        var res = await dashboardSb.from("guests").update(data).eq("id", id);
        if (res.error) throw res.error;
      } else {
        var existingGuest = await dashboardSb
          .from("guests")
          .select("id")
          .eq("slug", data.slug)
          .maybeSingle();
      ...
      if (!rsvpSection.classList.contains("d-none")) {
        ...
        if (rsvpId) {
          var rsvpRes = await dashboardSb
            .from("rsvps")
            .update(rsvpData)
            .eq("id", rsvpId);
          if (rsvpRes.error) throw rsvpRes.error;
        } else if (status) {
          var existingRsvp = await dashboardSb
            .from("rsvps")
            .select("id")
            .eq("guest_id", guestId)
            .maybeSingle();
          ...
          if (existingRsvp.data) {
            rsvpRes = await dashboardSb
              .from("rsvps")
              .update(rsvpData)
              .eq("id", existingRsvp.data.id);
          } else {
            rsvpRes = await dashboardSb.from("rsvps").insert([rsvpData]);
          }
          if (rsvpRes.error) throw rsvpRes.error;
        }
